...
const ContractAuditLogs = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/api/audit-logs/contract/${contractId}`)
      ...
      .catch(err => console.error("Failed to fetch contract-specific audit logs:", err));
  }, [contractId]);
  ...
  return (
    <div className={`min-h-screen p-6 ${styles.background} ${styles.text}`}>
      <div className={`max-w-6xl mx-auto p-6 rounded shadow ${styles.card}`}>
        <h1 className="text-2xl font-bold mb-6 text-[#3ab6de]">Audit Logs for Contract #{contractId}</h1>
        ...
        <div className="overflow-x-auto">
          ...
          {logs.length === 0 && (
            <p className="text-center text-gray-500 mt-4">No audit logs for this contract.</p>
          )}
        </div>
      </div>
    </div>
...
export default ContractAuditLogs;
